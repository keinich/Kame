...
namespace Kame {
  ...
  Mesh::Mesh()
    : m_IndexCount(0) {
    m_VertexBuffer = GraphicsCore::CreateVertexBuffer();
    m_IndexBuffer = GraphicsCore::CreateIndexBuffer();
  }
  ...
  //std::unique_ptr<Mesh> Mesh::CreateDebugCube(CommandListDx12& commandList, float size, bool rhcoords) {
  //  // A cube has six faces, each one pointing in a different direction.
  //  const int FaceCount = 6;
  ...
  void Mesh::Initialize(VertexCollection& vertices, IndexCollection& indices, bool rhcoords) {

    auto commandList = GraphicsCore::BeginCommandList(D3D12_COMMAND_LIST_TYPE_COPY);
    ...
    commandList->CopyVertexBuffer(m_VertexBuffer.get(), vertices);
    commandList->CopyIndexBuffer(m_IndexBuffer.get(), indices);
  ...
  void Mesh::Initialize(SimpleVertexCollection& vertices, IndexCollection& indices, bool rhcoords) {
    ...
    commandList->CopyVertexBuffer(m_VertexBuffer.get(), vertices);
    commandList->CopyIndexBuffer(m_IndexBuffer.get(), indices);
    GraphicsCore::ExecuteCommandList(commandList, true);
