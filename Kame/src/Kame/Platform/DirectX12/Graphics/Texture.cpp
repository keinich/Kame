...
#include "DX12Core.h"
#include "ViewDescriptionHelpers.h"
...
namespace Kame {
  ...
  void Texture::Resize(uint32_t width, uint32_t height, uint32_t depthOrArraySize) {
    ...
    if (_Resource1) {
      ResourceStateTracker::RemoveGlobalResourceState(_Resource1.Get());

      CD3DX12_RESOURCE_DESC resDesc(_Resource1->GetDesc());
  ...
  void Texture::CreateViews() {
    if (_Resource1) {
      ...
      CD3DX12_RESOURCE_DESC desc(_Resource1->GetDesc());
      ...
      if ((desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_RENDER_TARGET) != 0 &&
        ...
        _RenderTargetView = DX12Core::Get()->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_RTV);
        device->CreateRenderTargetView(_Resource1.Get(), nullptr, _RenderTargetView.GetDescriptorHandle());
      }
      if ((desc.Flags & D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL) != 0 &&
        CheckDSVSupport(formatSupport.Support1)) {
        _DepthStencilView = DX12Core::Get()->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_DSV);
        device->CreateDepthStencilView(_Resource1.Get(), nullptr, _DepthStencilView.GetDescriptorHandle());
      }
    }

    std::lock_guard<std::mutex> lock(_ShaderResourceViewsMutex);
    std::lock_guard<std::mutex> guard(_UnorderedAccessViewsMutex);
  ...
  DescriptorAllocation Texture::CreateShaderResourceView(const D3D12_SHADER_RESOURCE_VIEW_DESC* srvDesc) const {
    auto device = DX12Core::Get()->GetDevice();
    auto srv = DX12Core::Get()->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    device->CreateShaderResourceView(_Resource1.Get(), srvDesc, srv.GetDescriptorHandle());
  ...
  DescriptorAllocation Texture::CreateUnorderedAccessView(const D3D12_UNORDERED_ACCESS_VIEW_DESC* uavDesc) const {
    ...
    auto uav = DX12Core::Get()->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);

    device->CreateUnorderedAccessView(_Resource1.Get(), nullptr, uavDesc, uav.GetDescriptorHandle());
  ...
  D3D12_CPU_DESCRIPTOR_HANDLE Texture::GetShaderResourceView(const D3D12_SHADER_RESOURCE_VIEW_DESC* srvDesc) const {
    ...
    auto iter = _ShaderResourceViews.find(hash);
    if (iter == _ShaderResourceViews.end()) {
      auto srv = CreateShaderResourceView(srvDesc);
      iter = _ShaderResourceViews.insert({ hash, std::move(srv) }).first;
    }
  ...
  D3D12_CPU_DESCRIPTOR_HANDLE Texture::GetUnorderedAccessView(const D3D12_UNORDERED_ACCESS_VIEW_DESC* uavDesc) const {
    ...
    auto iter = _UnorderedAccessViews.find(hash);
    if (iter == _UnorderedAccessViews.end()) {
      auto uav = CreateUnorderedAccessView(uavDesc);
      iter = _UnorderedAccessViews.insert({ hash, std::move(uav) }).first;
    }
