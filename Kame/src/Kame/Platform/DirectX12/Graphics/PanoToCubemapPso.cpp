...
#include "Shaders/Compiled/PanoToCumbemap_CS.h"
...
namespace Kame {
  ...
  PanoToCubemapPSO::PanoToCubemapPSO() {
    ...
    _DefaultUAV = DX12Core::Get()->AllocateDescriptor(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV, 5);
    UINT descriptorHandleIncrementSize = DX12Core::Get()->GetDevice()->GetDescriptorHandleIncrementSize(D3D12_DESCRIPTOR_HEAP_TYPE_CBV_SRV_UAV);
    ...
    for (UINT i = 0; i < 5; ++i) {
      ...
      device->CreateUnorderedAccessView(nullptr, nullptr, &uavDesc, _DefaultUAV.GetDescriptorHandle(i));
    }
  }
