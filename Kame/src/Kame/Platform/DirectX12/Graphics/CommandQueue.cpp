...
namespace Kame {
  ...
  Reference<CommandListDx12> CommandQueue::GetCommandList() {
    Reference<CommandListDx12> commandList;
    ...
    if (!m_AvailableCommandLists.Empty()) {
      ...
      commandList = CreateReference<CommandListDx12>(m_CommandListType);
    }
  ...
  uint64_t CommandQueue::ExecuteCommandLists(const std::vector<Reference<CommandList> >& commandLists) {
    ...
    std::vector<Reference<CommandListDx12> > toBeQueued;
